...
# --- FUNCIÓN DE PROGRESO LITTERBOX ---
async def progress_bar_litterbox(current, total, status_msg, start_time):
    now = time.time()
    last_update = getattr(status_msg, "last_update_time", 0)
    ...
    text = (
        f"<b>📥 Descargando Video...</b>\n\n"
        f"<code>{bar}</code> {percentage:.1f}%\n"
        f"✅ <b>{current / 1024 / 1024:.1f} MB</b> de {total / 1024 / 1024:.1f} MB\n"
        f"🚀 <b>Velocidad:</b> {speed / 1024 / 1024:.1f} MB/s\n"
        f"⏳ <b>Restante:</b> {int(eta)} segundos"
    )
...
async def update_message(client, chat_id, message_id, text, reply_markup=None):
    try:
        await client.edit_message_text(chat_id, message_id, text, reply_markup=reply_markup, parse_mode=enums.ParseMode.HTML if "<b>" in text or "<code>" in text else enums.ParseMode.MARKDOWN)
...
# --- Lógica de Procesamiento y Litterbox ---

async def handle_litterbox_upload(client, chat_id, status_msg):
    user_info = user_data.get(chat_id)
    start_time = time.time()
    status_msg.last_update_time = start_time
    
    try:
        # Si ya está descargado (después de comprimir), lo usamos. Si no, descargamos.
        file_path = user_info.get('final_path') or user_info.get('download_path')
        
        if not file_path or not os.path.exists(file_path):
            await status_msg.edit_text("⏳ Iniciando descarga para enlace...")
            original_message = await client.get_messages(chat_id, user_info['original_message_id'])
            file_path = await client.download_media(
                original_message,
                progress=progress_bar_litterbox,
                progress_args=(status_msg, start_time)
            )
        
        await status_msg.edit_text("⚡ **Descarga completa.** Generando enlace de descarga...", parse_mode=enums.ParseMode.HTML)
        ...
        logger.error(f"Error Litterbox: {e}")
        await status_msg.edit_text(f"❌ Error crítico: {str(e)}")
    finally:
        clean_up(chat_id)
...
async def video_handler(client, message: Message):
    if filters.document(client, message) and not (message.document.mime_type and "video" in message.document.mime_type):
        return
    ...
    file_name = (message.video.file_name if message.video else message.document.file_name) or f"video_{message.id}.mp4"

    if file_size > MAX_VIDEO_SIZE_MB * 1024 * 1024:
        await message.reply(f"❌ El video supera el límite de {MAX_VIDEO_SIZE_MB} MB.")
        return
        
    user_data[chat_id] = {'state': 'awaiting_action', 'original_message_id': message.id, 'video_file_name': file_name, 'last_update_time': 0}
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔗 Obtener Enlace (Litterbox)", callback_data="action_litterbox")],
...
async def callback_handler(client, cb: CallbackQuery):
    ...
    if action == "cancel":
        ...
        clean_up(chat_id)
    elif action == "action_litterbox":
        await handle_litterbox_upload(client, chat_id, cb.message)
