...
async def live_status_loop(client, chat_id, message_id):
    """Tarea de fondo para actualizar el panel automáticamente cada 3 segundos."""
    while True:
        try:
            await asyncio.sleep(3)
            if WAITING_FOR_ID: continue
...
async def manager_callbacks(c, q):
    ...
    if data.startswith("t_"):
        ...
        await q.answer(f"🧹 Purga Completa", show_alert=True)
...
async def admin_input_handler(client, m):
    ...
    if WAITING_FOR_ID and m.text:
        ...
        if ids_found:
            ...
            await m.reply_text("❌ No encontré un ID válido.")
